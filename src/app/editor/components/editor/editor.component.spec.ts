...
fdescribe('#EditorModule EditorComponent', () => {
  ...
  it('#onSubmit() should raise the `whenSubmit` event with the forms value merged into one object', () => {
    const formsValue: webpadPost = {
      ...component.contentForm.getRawValue(),
      ...component.advancedOptionsForm.getRawValue()
    };
    let emittedValue;
    component.whenSubmit.subscribe(webpadPost => (emittedValue = webpadPost));
  ...
  it('#currentwebpadPost should be equal to a current merged value of both forms', () => {
    const expectedValue = {
      ...component.contentForm.getRawValue(),
      ...component.advancedOptionsForm.getRawValue()
    };
    const actualValue = component.currentwebpadPost;
