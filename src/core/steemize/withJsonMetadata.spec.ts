...
fdescribe('#core #steemize (json_metadata) withApp', () => {
  it('should return an object with `app` property which contains `webpad`', () => {
    const result = withApp({});

    expect(result.app).toContain('webpad');
  });
});
...
fdescribe('#core #steemize createJsonMetadata', () => {
  it('should create `json_metadata` with correct properties when `jsonMetadata` of `webpadPost` is empty', () => {
    const webpadPost: webpadPost = {
      title: '',
      body: 'any body',
      thumbnailUrl: '',
      tags: [],
    ...
    const jsonMetadata = createJsonMetadata(webpadPost);
...
fdescribe('#core #steemize withJsonMetadata', () => {
  ...
  it('should return an object with `json_metadata` property', () => {
    const result = withJsonMetadata(webpadPost)({});
  ...
  it('should NOT mutate the original `target` object', () => {
    ...
    withJsonMetadata(webpadPost)(target);
